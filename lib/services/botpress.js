...
export async function registerFileWithBotpress(file, title) {
  ...
  // Use the correct KB ID for "Documenten" knowledge base
  // The KB ID should be the actual ID of the "Documenten" knowledge base
  const kbId = BOTPRESS_CONFIG.KNOWLEDGE_BASE_ID;
  
  // File key format: kb-kbId/timestamp-filename.pdf
  const fileKey = `kb-${kbId}/${timestamp}-${filename}`;
  
  console.log(`📝 Registering file with key: ${fileKey}`);
  console.log(`📝 Using KB ID: ${kbId}`);
...
export async function getDocuments() {
  ...
  const kbId = BOTPRESS_CONFIG.KNOWLEDGE_BASE_ID;
  console.log(`📄 Fetching documents for KB ID: ${kbId}`);
  ...
  url.searchParams.append('tags[kbId]', kbId);
  ...
  console.log(`📋 Found ${files.length} files in knowledge base "${kbId}"`);
