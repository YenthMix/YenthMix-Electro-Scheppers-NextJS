...
interface LLMActivityChartProps {
  analytics: AnalyticsRecord[];
  dateRange: string;
}

export default function LLMActivityChart({ analytics, dateRange }: LLMActivityChartProps) {
  ...
  analytics.forEach((record) => {
    ...
    const existing = dataMap.get(dateKey) || { messages: 0, llmCalls: 0, llmErrors: 0 };
    dataMap.set(dateKey, {
      messages: existing.messages + (record.messages || 0),
      llmCalls: existing.llmCalls + (record.llm?.calls || 0),
      llmErrors: existing.llmErrors + (record.llm?.errors || 0)
    });
  });
  ...
  const chartData = generateDateRange().map((date) => {
    ...
    const data = dataMap.get(dateKey) || { messages: 0, llmCalls: 0, llmErrors: 0 };
  ...
  return (
    <div className="stat-box-chart-medium">
      <div className="box-header mb-4 pb-3 border-b border-gray-100">
        <div className="box-icon">🤖</div>
        <div className="box-title">LLM Activity</div>
      </div>
